# Q002 — Find Common Elements

## Original Source
`PDSA OPPE1/iitm-pdsa-main/LiveCoding/Week-2/W2LCP2-FindCommonElements.md`

## Original Question ID
`W2LCP2 — FindCommonElements`

## Concept
Finding common elements in two distinct-integer lists / sorting and two-pointer traversal
...
This is a **complete-code implementation question** in the original source. There is no `YOUR CODE HERE` marker in the source. The student is expected to write the required implementation of the supplied function.

```python
def findCommonElements(L1, L2):
    # Student writes the implementation here.
```
...
- `L1` and `L2` have the same length `n`.
- `L1` contains all distinct integers.
- `L2` contains all distinct integers.
- There can be many elements common to `L1` and `L2`.
- The returned list can be in any order.
- The required time complexity is O(nlogn) or better.
- Do not use the Python `set` built-in function.
